...
Citizen.CreateThread(function()
    ...
    local x, y, z = 197.35034179688,-992.91290283203,28.300422668457
    ...
    -- schimba hainele ped-ului
    SetPedComponentVariation(ped, 11, 116, 0, 2)  
    SetPedComponentVariation(ped, 8, 150, 0, 2)  
    SetPedComponentVariation(ped, 4, 57, 0, 2)   
    SetPedComponentVariation(ped, 6, 88, 1, 2)   
    SetPedComponentVariation(ped, 1, 8, 0, 2)   
...
Citizen.CreateThread(function()
    local x, y, z = 198.77647399902,-992.15563964844,28.30041885376 
    ...
    AddTextComponentString("Quest Craciun")
    ...
    while true do
        ...
        if not inPos then
            if #(vector3(196.48634338379,-994.57189941406,30.091764450073) - GetEntityCoords(PlayerPedId())) <= 5 then
                TriggerEvent("vrp-hud:showBind", {key = "F", text = "Apasa F Pentru a incepe quest-ul"})
                inPos = true
            end
        else
            if #(vector3(196.48634338379,-994.57189941406,30.091764450073) - GetEntityCoords(PlayerPedId())) > 5 then
           ...
           if not inColinda then
                if x ~= nil and y ~= nil and z ~= nil then
                    if #(vector3(x,y,z) - GetEntityCoords(PlayerPedId())) <= 5 then
                        while true do
                            Citizen.Wait(1)
                            TriggerServerEvent("glb:verificaLocul")
                            if zahar then
                            ...
                            if IsControlJustPressed(0, 23) and not zahar then
                                TriggerServerEvent("glb:ocupaLocul", "add", x,y,z)
                                ...
                                exports.glb_progressbar:Start("Colinzi", timp) -- Wait included
                                inColinda = false
                                zahar = false
                                triedOnce = false
                                TriggerServerEvent("glb:ocupaLocul", "remove")
                                theCp = theCp+1 
...
-- Events
RegisterNetEvent("glb:deschide", function(p) -- AICI DESCHIDE
    SendNUIMessage({type="open",portocale=p})
    SetNuiFocus(true,true)
end)
RegisterNetEvent("glb:cancel", function(val)
    if val == "not" then
        zahar = false
        return
    elseif val == "yes" then
...
RegisterNetEvent("glb:nextCheckpoint", function()
...
RegisterNUICallback("buy", function(data)
    TriggerServerEvent("glb:santaClaus", data)
end)
-- Events
-- Ped
-- Citizen.CreateThread(function()
